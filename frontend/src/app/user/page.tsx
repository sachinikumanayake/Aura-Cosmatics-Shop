    ...
    <div className="min-h-screen bg-[#FDFBFF] relative overflow-hidden font-sans pb-20">
      ...
      <div className="absolute inset-0 z-0 pointer-events-none">
        {flowers.map((flower, index) => (
          ...
          >
            <Image src="/images/flower-5r.png" alt="Background Flower" fill className="object-contain" />
          </motion.div>
        ))}
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-4 space-y-8">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white/70 backdrop-blur-2xl p-8 rounded-[40px] shadow-sm border border-white flex flex-col items-center text-center">
              <div className="relative mb-6">
                <div className="w-40 h-40 rounded-full p-1.5 bg-gradient-to-tr from-purple-200 to-pink-200">
                  <img src={getImageUrl(user.image)} className="w-full h-full object-cover rounded-full border-4 border-white shadow-sm" alt="Profile" />
                </div>
            ...
            <div className="bg-[#2D1B4E] p-8 rounded-[40px] text-white shadow-xl relative overflow-hidden group">
               ...
               <p className="text-purple-200 text-xs mb-6">Enjoy exclusive floral perks.</p>
               <button className="w-full py-3 bg-purple-500 text-white rounded-2xl font-bold text-sm hover:bg-purple-400 transition-colors">View Benefits</button>
            </div>
          </div>
          ...
          <div className="lg:col-span-8 space-y-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <motion.div 
                ...
                className="bg-white/60 p-6 rounded-3xl border border-white/50 flex items-center gap-4 cursor-pointer hover:bg-white/80 transition-all shadow-sm group"
              >
                <div className="p-3 bg-purple-50 text-purple-500 rounded-2xl group-hover:bg-purple-600 group-hover:text-white transition-colors">
                  <ShoppingBag size={20}/>
                </div>
                <div>
                  <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">My History</p>
                  <p className="text-lg font-bold text-[#2D1B4E]">Orders</p>
                </div>
                <ChevronRight size={16} className="ml-auto text-gray-300 group-hover:text-purple-500" />
              </motion.div>

              <div className="bg-white/40 p-6 rounded-3xl border border-white/50 flex items-center gap-4">
                <div className="p-3 bg-pink-50 text-pink-500 rounded-2xl"><Heart size={20}/></div>
                <div>
                  <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Wishlist</p>
                  <p className="text-lg font-bold text-[#2D1B4E]">24 Items</p>
                </div>
              </div>

              <div className="bg-white/40 p-6 rounded-3xl border border-white/50 flex items-center gap-4">
                <div className="p-3 bg-yellow-50 text-yellow-600 rounded-2xl"><Star size={20}/></div>
                <div>
                  <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Tier</p>
                  <p className="text-lg font-bold text-[#2D1B4E]">Silver</p>
                </div>
              </div>
            </div>
            ...
            <motion.div 
              ...
              className="bg-gradient-to-r from-[#2D1B4E] to-[#4A2B7E] p-8 rounded-[40px] text-white flex flex-col md:flex-row items-center justify-between gap-6 shadow-xl"
            >
              <div>
                <h3 className="text-2xl font-serif font-bold mb-2">Track Your Floral Aura</h3>
                <p className="text-purple-200 text-sm">Check status of your orders and updates.</p>
              </div>
              <button onClick={() => router.push('/my-orders')} className="px-8 py-4 bg-white text-[#2D1B4E] rounded-2xl font-bold hover:bg-purple-50 transition-colors shadow-lg">
                Go to My Orders
              </button>
            </motion.div>
            ...
            <div className="bg-white/50 backdrop-blur-md p-10 rounded-[50px] border border-white shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="h-48 bg-purple-50/50 rounded-[40px] flex flex-col items-center justify-center border border-purple-100">
                   <ShoppingBag className="text-purple-200 mb-2" size={40}/>
                   <p className="text-purple-400 font-medium">No items yet</p>
                </div>
                <div className="h-48 bg-gradient-to-br from-pink-50/80 to-purple-50/80 rounded-[40px] p-8 flex flex-col justify-end border border-white">
                   <p className="text-[#2D1B4E] font-bold text-lg leading-tight">Seasonal Fragrances</p>
                   <button className="text-xs font-black uppercase tracking-widest text-purple-600 self-start mt-4">Discover Now</button>
                </div>
              </div>
            </div>
          ...
          <>
            ...
            <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} className="fixed top-0 right-0 h-full w-full max-w-md bg-white z-[101] p-12 flex flex-col shadow-2xl">
              <div className="flex justify-between items-center mb-12">
                <h2 className="text-3xl font-serif font-bold text-[#2D1B4E]">Settings</h2>
              ...
              <div className="space-y-8 flex-1">
                ...
                <button 
                  onClick={() => router.push('/my-orders')}
                  className="w-full flex items-center gap-4 p-4 bg-purple-50 text-purple-700 font-bold rounded-2xl hover:bg-purple-100 transition-colors"
                >
